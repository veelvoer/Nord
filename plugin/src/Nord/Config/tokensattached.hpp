...
class Tokens : public QQuickAttachedPropertyPropagator, public QQmlParserStatus {
    ...
    Q_PROPERTY(const nord::config::AppearanceRounding* rounding READ rounding NOTIFY sourceChanged)
    Q_PROPERTY(const nord::config::AppearanceSpacing* spacing READ spacing NOTIFY sourceChanged)
    Q_PROPERTY(const nord::config::AppearancePadding* padding READ padding NOTIFY sourceChanged)
    Q_PROPERTY(const nord::config::AppearanceTransparency* transparency READ transparency NOTIFY sourceChanged)
    Q_PROPERTY(const nord::config::SizeTokens* sizes READ sizes NOTIFY sourceChanged)
    Q_PROPERTY(const nord::config::FontTokens* font READ font NOTIFY sourceChanged)
    Q_PROPERTY(const nord::config::AnimTokens* anim READ anim NOTIFY sourceChanged)
