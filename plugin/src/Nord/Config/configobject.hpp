...
public:                                                                                                                \
    ...
    void set_##name(const Type& val) {                                                                                 \
        if (nord::config::ConfigObject::updateMember(m_##name, val)) {                                            \
...
#define CONFIG_SUBOBJECT(Type, name)                                                                                   \
    Q_PROPERTY(nord::config::Type* name READ name CONSTANT)                                                       \
                                                                                                                       \
public:                                                                                                                \
    [[nodiscard]] Type* name() const {                                                                                 \
        return m_##name;                                                                                               \
...
public:                                                                                                                \
    [[nodiscard]] Type name() const {                                                                                  \
        if (isOverlay())                                                                                               \
            qCWarning(nord::config::lcConfig, "Reading global-only option '%s' on per-monitor overlay",           \
                qUtf8Printable(propertyPath(QStringLiteral(#name))));                                                  \
        return m_##name;                                                                                               \
    }                                                                                                                  \
    void set_##name(const Type& val) {                                                                                 \
        if (isOverlay())                                                                                               \
            qCWarning(nord::config::lcConfig, "Writing global-only option '%s' on per-monitor overlay",           \
                qUtf8Printable(propertyPath(QStringLiteral(#name))));                                                  \
        if (nord::config::ConfigObject::updateMember(m_##name, val)) {                                            \
            markPropertyLoaded(QStringLiteral(#name));                                                                 \
            Q_EMIT name##Changed();                                                                                    \
