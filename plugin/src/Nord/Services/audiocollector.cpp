...
Q_LOGGING_CATEGORY(lcAc, "nord.services.ac", QtInfoMsg)
Q_LOGGING_CATEGORY(lcAcWorker, "nord.services.ac.worker", QtInfoMsg)
...
PipeWireWorker::PipeWireWorker(std::stop_token token, AudioCollector* collector)
    ...
    m_stream = pw_stream_new_simple(pw_main_loop_get_loop(m_loop), "nord-shell", props, &events, this);
    if (!m_stream) {
